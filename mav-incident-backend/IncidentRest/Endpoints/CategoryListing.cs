...
{
    public class CategoryListing : RestService.Endpoints.RestEndpoint
    {

        public override string Path => "/category";
        ...
        protected override RestResponse process(HttpRequest request, Dictionary<string, string> urlParams)
        {
            ...
            List<CategoryListingEntry> categoryEntries = new List<CategoryListingEntry>();
            IncidentDatabase.Instance.Context.Categories
                .OrderBy(cat => cat.Name)
                .ToList()
                .ForEach(cat => categoryEntries.Add(new CategoryListingEntry(cat)));

            resp.Body = categoryEntries;
            return resp;
